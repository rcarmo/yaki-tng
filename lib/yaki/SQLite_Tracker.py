...
import os, sys, time, rfc822, unittest, urlparse, urllib, re, hashlib, sqlite3
...
class ReferrerCache:
  def __init__(self, ac, ignorelist = [], prunelist = []):
    ...
    """ CREATE TABLE referrers (
        ...
        CREATE UNIQUE INDEX referrer_index ON referrers(referrer, page);
        CREATE UNIQUE INDEX page_index ON referrers(page);
        CREATE INDEX date_index ON referrers(when);
        CREATE INDEX hit_index ON referrers(hits);
        """
  ...
  def add(self, referrer, page, now):
    ...
    VALUES (:referrer, :page, datetime(1092941466, 'unixepoch', 'localtime'),
      COALESCE(
          (SELECT hits FROM referrers 
                 WHERE referrer=:referrer and page=:pagesrc=:src),
                     0) + 1);"""
