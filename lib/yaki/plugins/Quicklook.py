...
template = """
<div class="quicklook_holder drop-shadow lifted" %(align)s><a title="%(title)s" href="%(large)s" class="quicklook"><img alt="%(alt)s" src="%(small)s" class="thumb"></a></div>
"""
...
class QuickLookWikiPlugin(yaki.Plugins.WikiPlugin):
  ...
  def run(self, serial, tag, tagname, pagename, soup, request, response):  
    params = {'serial':serial,'align':'','kind':''}
    ...
    for image in ['large','small']:
      ...
      if self.ac.store.isAttachment(pagename, params[image]):
        ...
        if image == 'small' and params['rel'] == 'screenshot':
            params[image] = c.pagethumb + pagename + "/" + params[image]
            continue
